		...
		if lookupErr != nil {
			if isNoBindingErr(lookupErr) {
				if sid, sidErr := currentSessionID(); sidErr != nil {
					...
					fmt.Fprintln(os.Stderr, "error: no task ref given and this harness session is not bound to a task — pass a slug or run `flow do --here <slug>` first")
				}
				return 1
			}
...
func showProjectCmd(args []string) int {
	...
	if ref == "" {
		...
		if lookupErr != nil {
			if isNoBindingErr(lookupErr) {
				if sid, sidErr := currentSessionID(); sidErr != nil {
					fmt.Fprintf(os.Stderr, "error: no project ref given and %v; pass a project ref explicitly\n", sidErr)
				} else if sid == "" {
					fmt.Fprintln(os.Stderr, "error: no project ref given and not running inside a known harness session")
				} else {
					fmt.Fprintln(os.Stderr, "error: no project ref given and this harness session is not bound to a task")
				}
				return 1
			}
			if strings.Contains(lookupErr.Error(), "multiple harness session env vars") {
